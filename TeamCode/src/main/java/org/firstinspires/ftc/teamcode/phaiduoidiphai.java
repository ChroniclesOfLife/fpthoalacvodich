...
@Autonomous(name = "PhaiDuoiDiPhai")
public class phaiduoidiphai extends LinearOpMode {
...
//        backRightMotor.setDirection(DcMotorSimple.Direction.REVERSE);
        ...
        if (opModeIsActive()) {
            ...
            while (distanceB.getDistance(DistanceUnit.CM) >= 20) {
                moveVerticalContinuous(0.15);
            }
            if (distanceB.getDistance(DistanceUnit.CM) <= 3) {
                stopAllMotors();
                double t = getRuntime();
                if (getRuntime() - t >= 1) {
                    dualArm.setPower(1);
